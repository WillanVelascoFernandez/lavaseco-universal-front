...
import { Washer, HistoryEntry } from '../../../types/machine';
import { washerService } from '../WasherService';
import { format } from 'date-fns';
import { es } from 'date-fns/locale';

interface WasherHistoryModalProps {
    isOpen: boolean;
    onClose: () => void;
    machine: Washer | null;
}

export const WasherHistoryModal: React.FC<WasherHistoryModalProps> = ({ isOpen, onClose, machine }) => {
    ...
    const fetchHistory = React.useCallback(async (page: number) => {
        ...
        setLoading(true);
        try {
            const data = await washerService.getWasherHistory(parseInt(machine.id), page, recordsPerPage);
                                ...
                                <div key={entry.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group">
                                    <div className="flex flex-row items-center justify-between gap-6">
                                        <div className="flex flex-col w-32 border-r border-gray-50">
                                            <div className="flex items-center gap-1.5 text-[9px] font-black text-brand-blue uppercase tracking-widest mb-1">
                                                <Calendar size={12} />
                                                {entry.date}
                                            </div>
                                        ...
                                        <div className="flex-1 border-r border-gray-50">
                                            <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Ciclo de Lavado</p>
                                        ...
                                        <div className="w-24 text-right">
                                            ...
                                            <Badge variant="info" className="text-sm border-none bg-blue-50 text-brand-blue">Bs. {entry.revenue.toFixed(2)}</Badge>
                                        </div>
                                    </div>
                                </div>
                            ))
