...
interface UserModalProps {
    ...
    onClose: () => void;
    user: User | null;
    onSave: (userData: Omit<User, 'id' | 'lastActive'>) => void;
}

export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, user, onSave }) => {
    const [name, setName] = React.useState('');
    const [email, setEmail] = React.useState('');
    const [role, setRole] = React.useState<'admin' | 'operador' | 'tecnico'>('operador');
    const [status, setStatus] = React.useState<'activo' | 'inactivo'>('activo');
